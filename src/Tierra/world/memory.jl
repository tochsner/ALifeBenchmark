...
function read_memory(model::TierraModel, address)
    model.memory[mod(address, model.memory_size)+one(UInt16)]
end
function read_memory(model::TierraModel, address, length)
    return [
        read_memory(model, curr_addr) for
        curr_addr = address:(address+length-one(UInt16))
    ]
end
function write_memory(
    model::TierraModel,
    organism::TierrianOrganism,
    address,
    instruction::UInt8,
)
    if organism_has_access(organism, address, 1)
        if MUTATE && rand() < FLAW_PROBABILITY
            instruction_to_write = get_mutated_instruction(instruction)
        else
            instruction_to_write = instruction
        end

        model.memory[mod(address, model.memory_size)+one(UInt16)] = instruction_to_write
...
function organism_has_access(organism::TierrianOrganism, address, length)
    if _is_within_range(
        address,
        length,
        organism.start_address,
        organism.length,
    )
        true
    elseif organism.has_daughter && _is_within_range(
        address,
        length,
        organism.daughter_address,
        organism.daughter_length,
    )
        true
    else
        false
...
function clear_memory!(model::TierraModel, start_address, length)
    ...
    for i = 1:Base.length(model.free_blocks)
        free_block = model.free_blocks[i]

        overlap = _determine_overlap(
            start_address,
            length,
            free_block.start_address,
            free_block.length,
        )
        ...
        elseif overlap == RightOverlap()
            return clear_memory!(
                ...
                start_address + length - free_block.start_address -
                free_block.length,
            )
    ...
    if index_left_neighbor != -1 && index_right_neighbor == -1
        ...
        model.free_blocks[index_left_neighbor] =
            FreeMemoryBlock(start_address, free_block.length + length)
    ...
    elseif index_left_neighbor == -1 && index_right_neighbor != -1
        ...
        model.free_blocks[index_right_neighbor] = FreeMemoryBlock(
            free_block.start_address,
    ...
    elseif index_left_neighbor != -1 && index_right_neighbor != -1
        ...
        model.free_blocks[index_left_neighbor] = FreeMemoryBlock(
            free_block_left.start_address,
            free_block_left.length + free_block_right.length + length,
        )
...
function allocate_free_memory!(model::TierraModel, length)
    ...
    for _ = 1:100
        ...
        if length <= block.length
            alloc_start, new_free_block =
                _allocate_in_block!(model, block, length)
